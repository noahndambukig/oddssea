...
| Plinko milestone | **Built, awaiting deploy + production suite** — the second game: three derived multiplier tables (exact RTPs 97.0703/97.0889/97.1521%, `decisions/0027`) with a load-time RTP-recompute tripwire; `bit_count` bucket derivation; the win-a-bet predicate fixed to `payout > stake`; migration 010 (tour-chain contract) paid; `challenge:play_two_games` dated 2026-08-09 grows the draw to 3. Games harness 14/14. Walkthrough Part 12 |
