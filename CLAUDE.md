...
| Crates milestone | **Built, not yet deployed** — all five crate kinds, starter grant (SQL-gated), barebones dex with transactional page/set bonuses, four pity mechanisms, full-draw audit trail (`decisions/0023`). Node pre-rolls, `open_crate()` decides under the lock; 006 also retrofits the 003 idempotency race. Selection harness 32/32 (rates ±0.5%, pity at exact boundaries). Awaiting deploy + production adversarial checks. Walkthrough Part 9 |
