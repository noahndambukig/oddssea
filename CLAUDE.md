...
| Crash milestone | **Built, in verification** — the first shared round (`decisions/0028`): time-indexed lazy rounds (round = UTC minute, bust = HMAC(secret, index) through `P(B≥m)=0.97/m`, uniform exact 97.00% multiplier RTP, whole-Shell floor published); both cash-out verbs, ties pay; decided-not-elapsed settlement; keyless settle with derived-state response; CrashRoundSecret RETAINed; ~1 s flight poll is the feed. Migration 012; harness 40/40 |
