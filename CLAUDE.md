...
| Closet milestone | **Built, awaiting deploy + production suite** — barebones equipping: `loadouts` (instance refs, preset-ready keys), `set_equipment` (no idempotency key — naturally idempotent, non-economic), `first_equipped_at` as event evidence, 4-step tour with the equip step, `first_equip` feature-first (`decisions/0025`). 009 is expand-only; **migration 010 (chain contract) is a forward obligation opening the NEXT milestone's branch**. Two-event UI bus. Walkthrough Part 11 |
