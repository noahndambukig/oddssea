...
1. **`01-game/core-loop.md`** — everything else assumes it and nothing has specced it. The revamp makes wagering the core activity (`decisions/0005`), but what players wager *on* — modes, events, where the odds come from — is unspecced.
2. **Rewrite `02-economy/` for the two-currency wager model** — bankroll-ruin simulation first to derive the numbers, then `currency-model.md` and `guardrails.md`. See roadmap question 6.
...
4. **`04-technical/data-model.md`** — the cosmetics spec implies a schema (items, skins, ownership, provenance, dex progress). Write it down before code starts guessing.
